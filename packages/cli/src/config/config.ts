...
export async function parseArguments(): Promise<CliArgs> {
  ...
  const yargsInstance = yargs(rawArgv)
    .locale('en')
    .scriptName('qwen-custom')
    .usage(
      'Usage: qwen-custom [options] [command]\n\nQwen Code - Launch an interactive CLI, use -p/--prompt for non-interactive mode',
    )
    .option('telemetry', {
      type: 'boolean',
      description:
...
export async function loadCliConfig(
  ...
  if (argv.continue || argv.resume) {
    ...
    if (argv.resume) {
      ...
      if (!sessionData) {
        const message = `No saved session found with ID ${argv.resume}. Run \`qwen-custom --resume\` without an ID to choose from existing sessions.`;
