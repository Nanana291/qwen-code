...
```bash
qwen-custom --continue --p "Continue with my task"
```
...
> qwen-custom --continue --p "Show me our progress"
>
> # Show conversation picker
> qwen-custom --resume
>
> # Continue most recent conversation in non-interactive mode
> qwen-custom --continue --p "Run the tests again"
> ```
...
// package.json
{
    ...
    "scripts": {
        ...
        "lint:Qwen Code": "qwen-custom -p 'you are a linter. please look at the changes vs. main and report any issues related to typos. report the filename and line number on one line, and a description of the issue on the second line. do not return any other text.'"
    }
}
```
...
```bash
cat build-error.txt | qwen-custom -p 'concisely explain the root cause of this build error' > output.txt
```
...
```bash
cat data.txt | qwen-custom -p 'summarize this data' --output-format text > summary.txt
```
...
```bash
cat code.py | qwen-custom -p 'analyze this code for bugs' --output-format json > analysis.json
```
...
```bash
cat log.txt | qwen-custom -p 'parse this log file for errors' --output-format stream-json
```
