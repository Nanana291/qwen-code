...
- By using `qwen-custom mcp` commands (see [CLI reference](#qwen-mcp-cli))
...
```bash
qwen-custom mcp add --transport http my-server http://localhost:3000/mcp
```
...
```bash
qwen-custom mcp add --scope user --transport http my-server http://localhost:3000/mcp
```
...
### Configure via `settings.json` vs `qwen-custom mcp add`
...
```bash
qwen-custom mcp add pythonTools -e DATABASE_URL=$DB_CONNECTION_STRING -e API_KEY=$EXTERNAL_API_KEY \
...
```bash
qwen-custom mcp add --transport http httpServerWithAuth http://localhost:3000/mcp \
...
```bash
qwen-custom mcp add --transport sse sseServer http://localhost:8080/sse --timeout 30000
```
...
- **Server shows “Disconnected” in `qwen-custom mcp list`**: verify the URL/command is correct, then increase `timeout`.
...
### Manage MCP servers with `qwen-custom mcp`
