...
> **Note for nvm users**: If you use nvm to manage Node.js versions, the SDK may not be able to auto-detect the Qwen Code executable. You should explicitly set the `pathToQwenExecutable` option to the full path of the `qwen-custom` binary.
...
| `pathToQwenExecutable`   | `string`                                       | Auto-detected    | Path to the Qwen Code executable. Supports multiple formats: `'qwen-custom'` (native binary from PATH), `'/path/to/qwen-custom'` (explicit path), `'/path/to/cli.js'` (Node.js bundle), `'node:/path/to/cli.js'` (force Node.js runtime), `'bun:/path/to/cli.js'` (force Bun runtime). If not provided, auto-detects from: `QWEN_CODE_CLI_PATH` env var, `~/.volta/bin/qwen-custom`, `~/.npm-global/bin/qwen-custom`, `/usr/local/bin/qwen-custom`, `~/.local/bin/qwen-custom`, `~/node_modules/.bin/qwen-custom`, `~/.yarn/bin/qwen-custom`. |
